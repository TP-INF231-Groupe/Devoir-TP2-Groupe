...
#include <stdlib.h>
#include <string.h>
#include "insertion_double_circulaire.h"


void display_menu(){
    printf("\n\n\n\t\t\t // INSERTION EN TETE ET EN QUEUE DANS UNE LISTE DOUBLEMENT CHAINEE //\n\n");
...
void display_doubly_circular_list(Node* head){
    if (!head){
        printf("[]");
        return;
    }
...
void insert_front_in_doubly_circular_list(Node** head, Node** tail, char data){
...
void insert_back_in_doubly_circular_list(Node** head, Node** tail, char data){
    Node* new_node = create_node(data);
    if (*head == NULL){
        new_node->next = new_node->prev = new_node;
        *head = new_node;
    ...
    while (1){
        ...
        if (n == 1){
            ...
            while (1){
                ...
                insert_front_in_doubly_circular_list(&head, &tail, data);
                while (getchar() != '\n');
            }
            ...
            while (1){
                ...
                insert_back_in_doubly_circular_list(&head, &tail, data);
                while (getchar() != '\n');
            }
        }
        else if(n == 3){
            printf("\t  La liste est: ");
            display_doubly_circular_list(head);
        }else break;
    }
}
